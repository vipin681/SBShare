...
app.controller("AngularJs_Controller", function ($scope, $timeout, $rootScope, $window, $http) {
    ...
    function selectuerRoleDetails(UserRole) {      
        $http.get(VirtualDirectoryPath + '/api/User/getUserRoleDetails/', { params: { UserRole: UserRole } }).success(function (data) {
  ...
  .error(function () {
    ...
    $scope.MenuDelete = function MenuDelete(MenuIdentity, menuName) {
        ...
        if (delConfirm == true) {
            ...
            $http.get(VirtualDirectoryPath + '/api/User/deleteMenu/', { params: { MenuIdentity: $scope.MenuIdentitys } }).success(function (data) {
    ...
    $scope.saveDetails = function () {
        ...
        if ($scope.IsFormValid) {
            ...
            if ($scope.MenuIdentitys == 0) {
                ...
                $http.get(VirtualDirectoryPath + '/api/User/insertMenu/', { params: { menuID: $scope.menuIDs, menuName: $scope.menuNames, parentMenuID: $scope.parentMenuIDs, UserRole: $scope.selectedUserRole, menuFileName: $scope.menuFileNames, MenuURL: $scope.MenuURLs, UseYN: $scope.UseYNsN } }).success(function (data) {
         ...
         .error(function () {
             $scope.error = "An Error has occured while loading posts!";
                ...
                $http.get(VirtualDirectoryPath + '/api/User/updateMenu/', { params: { MenuIdentity: $scope.MenuIdentitys, menuID: $scope.menuIDs, menuName: $scope.menuNames, parentMenuID: $scope.parentMenuIDs, UserRole: $scope.selectedUserRole, menuFileName: $scope.menuFileNames, MenuURL: $scope.MenuURLs, UseYN: $scope.UseYNsN } }).success(function (data) {
    ...
    function selectMenubyUserRoleDetails(UserRole) {
       // alert($scope.userRoleName);
        $http.get(VirtualDirectoryPath + '/api/User/getMenubyUserRole/', { params: { UserRole: $scope.userRoleName } }).success(function (data) {
